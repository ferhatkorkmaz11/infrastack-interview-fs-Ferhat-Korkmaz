...
const TracesContent: React.FC = () => {
  ...
  const [spanKindFilter, setSpanKindFilter] = useState(searchParams?.get('spanKind') || '');
  const [spanNameFilter, setSpanNameFilter] = useState(searchParams?.get('spanName') || '');
  const [statusCodeFilter, setStatusCodeFilter] = useState(searchParams?.get('statusCode') || '');
  ...
  }, [page, serviceFilter, spanKindFilter, spanNameFilter, statusCodeFilter, sortOrder]);
  ...
  const updateURL = useCallback(() => {
    ...
    if (spanKindFilter) newParams.set('spanKind', spanKindFilter);
    if (spanNameFilter) newParams.set('spanName', spanNameFilter);
    if (statusCodeFilter) newParams.set('statusCode', statusCodeFilter);
    newParams.set('sortOrder', sortOrder);
    newParams.set('page', page.toString());
    router.push(`/traces?${newParams.toString()}`);
  }, [searchTerm, serviceFilter, spanKindFilter, spanNameFilter, statusCodeFilter, sortOrder, page, router]);
  ...
  const handleFilterChange = (filterType: 'service' | 'spanKind' | 'spanName' | 'statusCode', value: string) => {
    setPage(1);
    switch (filterType) {
      case 'service':
  ...
  const handleSeeLogsClick = (traceId: string, event: React.MouseEvent) => {
    event.stopPropagation(); 
    const logsUrl = `/logs?search=${traceId}`;
    ...
    <div className="min-h-screen bg-background">
      <main className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <h1 className="text-3xl font-bold text-foreground mb-6">
          {serviceFilter ? `Traces for ${serviceFilter}` : 'All Traces'}
        </h1>
              ...
              <li key={trace.SpanId} className="px-6 py-4 hover:bg-secondary/10 cursor-pointer" onClick={() => openTraceDetails(trace.TraceId)}>
                ...
                <div className="mt-2 sm:flex sm:justify-between items-center">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <button
                      onClick={(e) => handleSeeLogsClick(trace.TraceId, e)}
